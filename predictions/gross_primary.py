...
def extract_gross_data(tiff_file, tfw_file=None):
...
def process_gross_data(input_folder, output_folder):
    ...
    for year in range(2010, 2024):
        tiff_file = os.path.join(input_folder, f"{year}_GP.tif")
        tfw_file = None

        if os.path.exists(tiff_file):
            print(f"Elaborazione: {tiff_file}")

            df = extract_gross_data(tiff_file, tfw_file)
...
def plot_gross():
    ...
    years = list(range(2011, 2024))  # From 2011 to 2023
    file_paths = {year: f"data/gross_primary/{year}.csv" for year in years}
    ...
    cbar.set_label("gross (mm)")

    # Set title and labels
    ax.set_title(f"gross Data ({year_init})")
    ...
    def update(val):
        ...
        ax.set_title(f"gross Data ({year})")  # Update title
...
input_folder = output_folder = "data/gross_primary"
process_gross_data(input_folder, output_folder)
plot_gross()