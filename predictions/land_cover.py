...
def extract_land_data(tiff_file, tfw_file=None):
    ...
    if not os.path.exists(tfw_file):
        print(f"TFW file not found: {tfw_file}")
...
def process_land_data(input_folder, output_folder):
    ...
    for year in range(2010, 2024):
        ...
        if os.path.exists(tiff_file):
            ...
            df = extract_land_data(tiff_file, tfw_file)
...
def plot_land():
    ...
    years = list(range(2010, 2024))  # From 2011 to 2023
    file_paths = {year: f"data/land_cover/{year}.csv" for year in years}
    # Load initial data
    year_init = 2011
    ...
    cbar.set_label("Land Cover (%)")

    # Set title and labels
    ax.set_title(f"land Data ({year_init})")
    ...
    def update(val):
        year = int(slider.val)
        df = pd.read_csv(file_paths[year]).dropna(subset=["lon", "lat", "value"])
        scatter.remove()  # Rimuove il vecchio scatter
        scatter = ax.scatter(df["lon"], df["lat"], c=df["value"], cmap="viridis", s=50, transform=ccrs.PlateCarree())
        ax.set_title(f"land Data ({year})")
...
input_folder = output_folder = "data/land_cover"
process_land_data(input_folder, output_folder)
plot_land()
